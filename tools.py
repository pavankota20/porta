#!/usr/bin/env python3
"""
Async LangChain tools for Porta Finance Assistant
"""
...
async def add_to_portfolio(user_id: str = DEFAULT_USER_ID, ticker: str = "",
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(PORTFOLIO_API_URL, json=payload, headers={"Content-Type": "application/json"}, timeout=aiohttp.ClientTimeout(total=10)) as response:
                print(f"[LOG] API response status: {response.status}")
                ...
                if response.status in [200, 201]:  # 200 OK, 201 Created
                    ...
                    print(f"[LOG] API response unexpected status: {response.status}")
...
async def remove_from_portfolio(user_id: str = DEFAULT_USER_ID, ticker: str = ""):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(list_url, timeout=aiohttp.ClientTimeout(total=10)) as list_response:
                print(f"[LOG] List API response status: {list_response.status}")
                ...
                async with session.delete(delete_url, timeout=aiohttp.ClientTimeout(total=10)) as delete_response:
                    print(f"[LOG] Delete API response status: {delete_response.status}")
...
async def list_portfolio(user_id: str = DEFAULT_USER_ID):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                ...
                if response.status == 200:
                    ...
                    print(f"[LOG] API response failed with status {response.status}")
                    try:
                        ...
                        return {"ok": False, "error": f"Unable to retrieve portfolio (HTTP {response.status})"}
...
async def get_portfolio_summary(user_id: str = DEFAULT_USER_ID, include_pnl: bool = True):
            ...
            async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        return {"ok": False, "error": f"Unable to retrieve portfolio summary (HTTP {response.status})"}
...
async def add_to_watchlist(user_id: str = DEFAULT_USER_ID, ticker: str = "",
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(WATCHLIST_API_URL, json=payload, headers={"Content-Type": "application/json"}, timeout=aiohttp.ClientTimeout(total=10)) as response:
                print(f"[LOG] API response status: {response.status}")
...
async def remove_from_watchlist(user_id: str = DEFAULT_USER_ID, ticker: str = ""):
...
async def list_watchlist(user_id: str = DEFAULT_USER_ID):
            ...
            async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        return {"ok": False, "error": f"Unable to retrieve watchlist (HTTP {response.status})"}
...
async def get_watchlist_entry(user_id: str = DEFAULT_USER_ID, ticker: str = ""):
            ...
            async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        return {"ok": False, "error": f"Unable to retrieve watchlist entry (HTTP {response.status})"}
...
async def get_news(user_id: str = DEFAULT_USER_ID, ticker: str = "", lookback_days: int = 3):
